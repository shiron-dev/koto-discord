...
class SetCommand @Autowired constructor(
    ...
    override val commandOptions: List<OptionData>
        get() = listOf(
            OptionData(
                OptionType.CHANNEL,
                "text",
                messages.getMessage("command.option.vc_notification.set.text", arrayOf(), Locale.JAPAN),
                true
            ),
            OptionData(
                OptionType.CHANNEL,
                "vc",
                messages.getMessage("command.option.vc_notification.set.vc", arrayOf(), Locale.JAPAN)
            )
        )
    ...
    override fun onSlashCommand(cmd: BotSlashCommandData) {
        ...
        if (text == null) {
            cmd.reply(messages.getMessage("command.error.internal", arrayOf(), Locale.JAPAN))
            return
        }
        ...
        cmd.reply(messages.getMessage("command.message.vc_notification.set.success", arrayOf(vc?.asMention ?: "サーバー全体", text.asMention), Locale.JAPAN))
    }
}
